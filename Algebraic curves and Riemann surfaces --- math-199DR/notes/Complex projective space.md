...
**Complex projective $n$-space** is the complex manifold $\mathbb{C} \mathbb{P}^n$ parameterising one-dimensional subspaces of $\mathbb{C}^{n + 1}$ and given by $\mathbb{C} \mathbb{P}^n = (\mathbb{C}^{n + 1} \setminus \{ 0 \}) / \mathbb{C}^\times$.
...
Note that by scaling, we can always choose all $x_{j} / x_{i}$ on $U_{i}$ to be less than $1$. But then, $\mathbb{C} \mathbb{P}^n$ is covered by $n + 1$ compact discs, and thus, is compact itself.
