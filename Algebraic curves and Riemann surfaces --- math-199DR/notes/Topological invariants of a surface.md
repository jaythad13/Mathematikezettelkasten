...
The **genus** $g_{X}$ (or $g$ if the surface is clear) of a (connected) surface $X$, is the maximum number of disjoint simple closed curves that can be removed from the surface while leaving it connected.
...
A **triangulation** $X_{T}$ of a surface $X$ is a collection of closed subsets $\{ \Delta \}$ of $X$, each homeomorphic to a triangle such that any two triangles are either disjoint, intersect only at a single vertex, or intersect at a single edge.

---

##### _definition:_ Euler characteristic of a triangulation, Euler characteristic of a surface

The **Euler characteristic of a triangulation** $X_{T}$ is $\chi(X_{T}) = \#V - \#E + \#F$, where $\#V$ is the number of vertices, $\#E$ is the number of edges, and $\#F$ is the number of faces in the triangulation.
