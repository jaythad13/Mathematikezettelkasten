...
Given a [[Simplicial homology and random walks --- math-145/notes/Calculus on a graph#_definition _ graph with boundary|graph with boundary]] $G$ with $g : B(G) \to \mathbb{R}$ a "reward" function. A random walk is a random sequence of vertices $x_{0}, x_{1}, \dots$ defined by
$$
x_{n + 1} = \begin{cases}
...
If the random walk is eventually constant at $b \in B(G)$, we say the walk stops at $b$. We say then that the reward is $g(b)$. It's a fact that for a finite graph with [[Simplicial homology and random walks --- math-145/notes/Calculus on a graph#_definition _ relative Betti number|relative Betti number]] $\mathrm{b}_{0}(G, B) = 0$, (that is, all connected components eventually reach the boundary) then stopping is probability $1$ (almost certain).
...
$$
where $b$ is the random variable that gives the vertex where random walk stops (so $g(b)$ is just a shorthand for reward), then $f$ solves [[Simplicial homology and random walks --- math-145/notes/The Dirichlet problem on a graph|the Dirichlet problem on a graph]] with boundary value $g$.
...
We can also use our tools to understand random walks on infinite graphs, as long as they are [[Simplicial homology and random walks --- math-145/notes/Calculus on a graph#_definition _ locally finite graph|locally finite]].
...
Our heuristic to understand transience and recurrence will be then to estimate the probabilities on finite subgraphs, using the solution to the [[Simplicial homology and random walks --- math-145/notes/The Dirichlet problem on a graph#The weighted Dirichlet problem|(weighted) Dirichlet problem]] on the graph. In analogy with the applications of the Dirichlet problem, this can be seen as estimating the effective resistance between the starting vertex and infinity.
