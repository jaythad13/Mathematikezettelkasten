...
The argument of $\mathbf{v} = (v_{1}, v_{2}) \in \mathbb{R}^2 \setminus \{ 0 \}$, is $\operatorname{arg} \mathbf{v}$, the equivalence class of all real $\theta$ such that $\lVert \mathbf{v} \rVert e^{i \theta}z = v_{1} + i v_{2}$ in the [[Abstract algebra --- math-171/notes/Fibres and quotients#_definition, theorem _ the quotient group is a group|quotient group]] $\mathbb{R} / 2 \pi \mathbb{Z}$.
