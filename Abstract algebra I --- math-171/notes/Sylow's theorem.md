...
We have seen the action $G \circlearrowright G$ (in particular, by [[Abstract Algebra I --- math-171/notes/The class equation#$G$ acting on itself by conjugation|conjugation]]. Here we will need consider the action of $G$ on a set of subsets. In particular, if $X$ is a set of subgroups of $G$, consider the action of $G$ on $X$ by left multiplication
$$
g \cdot H = g H
$$
and by conjugation
...
We claim $P$ is the only fixed point of this action. Suppose $Q$ is a fixed point of the action giving $p Q p ^{-1} = Q$ for any $p \in P$. Thus, the [[Abstract Algebra I --- math-171/notes/Centralisers, centre, and normalisers#_definition _ normaliser, $N_{G}(A)$|normaliser]] of $Q$ contains all $p \in P$ (as well as all $q \in Q$). But these are both Sylow $p$-subgroups of $N_{G}(Q)$ (they have to be $p$-subgroups of maximal size since they are maximal in $G$). But then $P = g Q g^{-1}$ for some $g \in N_{G}(Q)$ and since $g \in N_{G}(Q)$, this just means $P = Q$.
