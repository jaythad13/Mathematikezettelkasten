...
Recall the [[Curves and surfaces --- math-142/notes/Vector fields#_(temporary) definition _ curves, paths in $ mathbb{R} 3$|temporary definition of a curve]] that we gave last time. If $\alpha$ is a function that defines a curve $c$, then we say that $\alpha$ parameterises $c$. Note that we can write $\alpha$ as having component functions $\alpha_{1}, \alpha_{2}, \alpha_{3}$.
