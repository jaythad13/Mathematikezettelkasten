...
Now that we have a good idea of [[Curves and surfaces --- math-142/notes/Surfaces|surfaces]], we can start to calculus on them, like we would want to (perhaps, for physics, or just for math's own sake). Basically, the openness of each of the domains of the [[Curves and surfaces --- math-142/notes/Surfaces#_definition _ coordinate patch|coordinate patches]] means that we can take derivatives and limits inside them, and then we will try in some sense to transfer them to the neighbourhoods of the points on the surface.
...
To actually compute derivatives however, we need a [[Curves and surfaces --- math-142/notes/Tangent spaces on surfaces|notion of directions on the surface]], which we can finally do!
...
Finally, the notion of [[Curves and surfaces --- math-142/notes/Differential forms on a surface|differential forms on a surface]] gives us a natural pullback $F^* : \Omega^k(N) \to \Omega^k(M)$.
