...
With Frenet-Serret frames, we are setting up the [[Curves and surfaces --- math-142/notes/Frames|frame]]work for a lot of differential geometry.
...
If $\alpha$ is a [[Curves and surfaces --- math-142/notes/Curves#_definition _ regularity of a curve|regular curve]], there is an orientation preserving reparameterisation $\beta = \alpha \circ h$ such that $\lVert \beta'(\tau) \rVert = 1$ for all $\tau \in J$.
...
Recall how a [[Curves and surfaces --- math-142/notes/Frames#_definition _ vector field on a curve|vector field on a curve]] allows us to assign any tangent vector part to each point on the curve. To get from this to a frame, we need a made up definition.
...
For any strongly regular curve parameterised by $\alpha$ the vector fields $\{ \mathbf{T}, \mathbf{N}, \mathbf{B} \}$ form a [[Curves and surfaces --- math-142/notes/Frames#_definition _ frame|frame]], where
$$
\begin{gathered}
