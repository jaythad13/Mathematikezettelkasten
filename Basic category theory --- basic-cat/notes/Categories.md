...
2) $\mathsf{Grp}$ is a category with [[Abstract Algebra I --- math-171/notes/Motivating groups#_definition _ group, identity, inverse|groups]] as objects and [[Abstract Algebra I --- math-171/notes/Group homomorphisms#_definition _ group homomorphisms|group homomorphisms]] as arrows, and similarly, $\mathsf{Ring}$ is a category with rings as objects and ring homomorphisms as arrows, and $\mathsf{Vect}_{k}$ is a category with vector spaces as objects and linear maps as arrows. We can define similarly, $\mathsf{CRing}$ for commutative rings, $\mathsf{Field}$ for fields, and $\mathsf{Mod}_{R}$ for modules over commutative $R$ (or specify with some similar symbol the category of left/right modules over non-commutative $R$)
