...
\begin{solution}
	...
	\[
		...
		\end{tikzcd}
	\]
	That is, the pair $(\ker \theta, i)$ satisfy the universal property that for all groups $K$ with a homomorphism, $\varphi : K \to G$ such that ${\theta \circ \varphi = \varepsilon \circ \varphi}$, there is a unique homomorphism $\varphi' : K \to \ker \theta$ such that $i \circ \varphi' = \varphi$.
...
\begin{solution}
	For a topological space $X$, covered by two open subsets, $U$ and $V$ and another topological space $Y$, we claim that if the diagram without the dashed arrow commutes, then there is a unique continuous function $h$ such that the diagram with the dashed arrow commutes.
	\[
		\begin{tikzcd}
...
\begin{solution}
	\begin{enumerate}
		\item For any polynomial $p = \sum_{i = 1}^n a_i x^i$ consider the ring homomorphism given by
			...
			\]
			This is clearly a ring homomorphism (we verified that the restriction $\varphi_{| \mathbb{Z}}$ is a ring homomorphism already) and obviously we have that ${\varphi(x) = r}$.

			We can show that this is unique by noticing that for any homomorphism $\psi$ fulfilling the requirements, we must have that $\psi_{| \mathbb{Z}}$ is the unique homomorphism $\psi_{|\mathbb{Z}} : \mathbb{Z} \to R$ and thus, is as described, and since $\psi(x) = r$, we have
			\[
				\begin{split}
...
\begin{solution}
	\begin{enumerate}
		...
		\item
			The desired cone corresponds to the following diagram.
			\[
				\begin{tikzcd}
		...
		\item
			Suppose $(Q', q_{1}', q_{2}')$ is another cone with the same property. Then notice that the following diagrams commute, where $q, q'$ are the unique maps defined by the property.
			\[
				\begin{tikzcd}
				...
				\end{tikzcd}
			\]
			Notice then that $q' \circ q$ is a map on $Q$ such that for the cone $(Q, q_{1}, q_{2})$, we have 
			\[
				\begin{split}
				...
				\end{split}
			\]
			for $j \in \mathbb{N}_{2}$. Similarly, we can show that $(q \circ q')$ is a map on $Q'$ that gives us $(q \circ q') \circ q_{j}' = q_{j}'$ for all $j \in \mathbb{N}_2$. But we already have that $\operatorname{id}_{Q}$ is the unique map satisfying this on $Q$ and $\operatorname{id}_{Q'}$ for $Q'$. Thus, we must have $(q' \circ q) = \operatorname{id}_{Q}$ and $(q \circ q') = \operatorname{id}_{Q'}$. Thus, $Q \cong Q'$. That is, any other cone that satisfies this property is essentially the same as $Q$.
	\end{enumerate}
\end{solution}
