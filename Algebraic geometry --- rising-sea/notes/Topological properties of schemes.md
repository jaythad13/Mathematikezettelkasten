...
All truly nice schemes should be qcqs — like projective schemes.

##### _example:_ projective schemes are qcqs

Let $X = \operatorname{Proj} S_{\bullet}$ be a [[Algebraic geometry --- rising-sea/notes/Projective schemes#_definition _ projective $A$-schemes, quasiprojective $A$-schemes|projective scheme]]. Since $S_{\bullet}$ is a finitely generated graded ring over, $S_{+}$ is finitely generated by elements of positive degree — say $S_{+} = (x_{1}, \dots, x_{n})$. [[Algebraic geometry --- rising-sea/notes/Projective schemes#_proposition _ the irrelevant ideal really is irrelevant|Thus]], $X$ is the union of finitely many affine opens $D_{+}(x_{i})$, [[Algebraic geometry --- rising-sea/notes/The base of distinguished open sets#_corollary _ every affine scheme is quasicompact|each of which is quasicompact]]. Thus, $X$ is quasicompact. Each of the intersections of these affine opens is $D_{+}(x_{i} x_{j})$ which is itself an affine open. Thus, $X$ is also quasiseparated.
