...
Just as taking $\operatorname{Mor}$ or $\operatorname{Hom}$ [[Algebraic geometry --- rising-sea/notes/Functors#_example _ the functor of points and its opposite|is functorial]], so is $\mathcal{Hom}$. In particular, for $\psi : \mathscr{G} \to \mathscr{H}$ we get a sheaf morphism $\mathcal{Hom}(\mathscr{F}, \mathscr{G}) \to \mathcal{Hom}(\mathscr{F}, \mathscr{H})$. On each open set $U$ it is given by $h^{\mathscr{F}_{\mid U}}(\psi_{\mid U}) : \operatorname{Mor}(\mathscr{F}_{\mid U}, \mathscr{G}_{U}) \to \operatorname{Mor}(\mathscr{F}_{\mid U}, \mathscr{H}_{\mid U})$. That is, on $U$ we compose with $\psi_{\mid U}$. This is a sheaf morphism essentially because restricting to smaller open sets commutes with composition of sheaf morphisms. It is functorial for the same reason, applied in a different way.