...
This is also injective, even after quotienting out by the action of $\mathbb{F}^\times$. Suppose $(1 : a_{1} : \dots : a_{n}) \sim (b_{0} : 1 : b_{2} : \dots : b_{n})$. That is, there is some $\lambda$ such that $\lambda b_{0} = 1$, $\lambda = a_{1}$, and $\lambda b_{i} = a_{i}$. Thus, $b_{i} = a_{i} / a_{1}$ for $i \geq 2$. We want to show that $(x_{1 / 0} - a_{1}, \dots, x_{n} - a_{n / 0}) \in U_{0}$ and $(x_{0 / 1} - b_{0 / 1}, x_{2 / 1} - b_{2 / 1} \dots, x_{n / 1} - b_{n / 1}) \in U_{1}$ are the same point. Localising, we get exactly this by our theorem on [[Algebraic geometry --- rising-sea/notes/Morphisms of affine schemes#_example _ coordinates for an affine scheme morphism|coordinates for morphisms of affine schemes]] over $\mathbb{F}$.
