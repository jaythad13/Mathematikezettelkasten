...
Suppose $S_{\bullet}$ is generated over $S_{0}$ by $x_{1}, \dots, x_{n}$ of degrees $d_{1}, \dots, d_{n}$. Let $m = n \prod d_{i}$. Suppose $x = \prod x_{i}^{e_{i}}$ has degree at least $m$. That is, $\sum d_{i} e_{i} \geq n \prod d_{i}$. Then there must be some $i$ such that $d_{i} e_{i}$ is at least the average degree — such that $e_{i} \geq \prod_{j \neq i} d_{j}$. Write $c_{i} = \prod_{j \neq i} d_{j}$ and write $y_{i} = x_{i}^{c_{i}}$. Note, $\deg y_{i} = m / n$. 

We claim $S_{m \bullet}$ is generated by all finitely many degree $m$ monomials in the $x_{i}$. We will show each monomial $x = \prod x_{i}^{e_{i}}$ of degree $dm$ ("new degree" $d$) is a product of such monomials. Since $\deg x = dm \geq m$, there is some $y_{i}$ dividing $x$. In fact, we can factor out $n (d - 1)$ of these $y_{i}$ and are left with a monomial of degree $m$. The product of all the $y_{i}$ factored out has degree $(m / n) n (d - 1) = (d - 1) m$ and so can be factored into monomials of degree $m$ in the $x_{i}$.
