...
Suppose $M'$ is some distinct $A$-module with maps from each $M_{i}$ making the desired diagrams commute. Again, the maps of each $M_{i} \to M'$ must factor (uniquely) through the coproduct $M = \bigoplus_{i \in \mathscr{I}} M_{i}$. The maps $M \to M'$ factor uniquely through $U$ since $m_{j} - f(m_{j}) \mapsto 0$ for all $m_{j} \in M_{j}$ and $f : M_{j} \to M_{k}$. This is true because the map $M_{j} \to M'$ factors as $m_{j} \mapsto f(m_{j}) \mapsto m'$ for $M_{k} \to M'$ by $f(m_{j}) \to m'$.

##### _proposition:_ localisation is a colimit

Suppose $S$ is a multiplicative subset and $A$ is an [[Abstract algebra --- math-171/notes/Integral domains#_definition _ integral domain|integral domain]], then the [[Algebraic geometry --- rising-sea/notes/Localisation, categorically#_definition _ localisation of a ring, localisations at a prime, fraction field|localisation]] $S^{-1} A$ can be interpreted as $\operatorname{colim} \frac{1}{s} A$ where the colimit is over $s \in S$ and in the category of $A$-modules.

###### _proof:_

Note that if $s_{1} d = s_{2}$ we have a map $\frac{1}{s_{1}} A \to \frac{1}{s_{2}} A$ by $a / s_{1} \mapsto a d / s_{2}$. We take the colimit over the diagram containing all such maps. This colimit is $\bigoplus_{s \in S} \frac{1}{s} A / U$ where $U$ is the submodule generated by all $a / s_{1} - ad / s_{2}$ with $s_{2} = s_{1} d$. But this corresponds exactly to the [[Algebraic geometry --- rising-sea/notes/Localisation, categorically#_definition _ localisation of a ring, localisations at a prime, fraction field|explicit construction]] of $S^{-1} A = \{ a / s \mid a \in A, s \in S \} / \sim$ with $a_{1} / s_{1} \sim a_{2} / s_{2}$ if and only if $a_{1} s_{2} = a_{2} s_{2}$. The implied definition of addition here (as addition in $\frac{1}{m} A$ for $m$ any common multiple of $s_{i}$) agrees with the explicit definition of addition in the $S^{-1} A$. Scaling by $a \in A$ is obviously the same.
