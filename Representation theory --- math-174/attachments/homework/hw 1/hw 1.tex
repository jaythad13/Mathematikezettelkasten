...
We claim there are $2^{\aleph_{0}}$-many left ideals (left submodules of $R$ are exactly left ideals of $R$). To see this, consider $T, S \in R$ and the corresponding principal ideals. If $T \in (S)$, then $T = AS$ for some $A \in R$, and so  $\ker T$ includes $\ker S$. Thus, if $(S) = (T)$, then $\ker T = \ker S$. That is, the kernel of the generator is an invariant of principal left ideals.

Each subspace of $\mathbb{C}^{2}$ is represented as a kernel of some matrix in $R$, and there are $2^{\aleph_{0}}$-many subspaces (for example, there is one subspace for each slope $\lambda \in \mathbb{C}$). There are at least as many left ideals as there are subspaces of $\mathbb{C}^{2}$. Each left ideal of $R$ is also a $\mathbb{C}$-submodule and there are at most $2^{\aleph_{0}}$-many of those (the best way I can think to see this is by enumerating the points of the complex Grassmannian, but that seems overkill). Thus, there are exactly $2^{\aleph_{0}}$-many left ideals of $R$.
