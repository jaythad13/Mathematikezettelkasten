...
We've seen enough [[Abstract algebra --- math-171/notes/Fibres and quotients|examples]] of quotient groups. Now we want to see whether it makes sense to quotient out by any subgroup or there are some restrictions on what we can quotient out by.

### Normal subgroups

[[Abstract algebra --- math-171/notes/Fibres and quotients#_definition, theorem _ the quotient group is a group|We've seen]] that there's a natural group structure on the cosets of a homomorphism kernel. Do we need a subgroup to be a kernel to quotient out by it in that way? Can all subgroups be kernels? Yes and no.
...
Now we want to relate [[Abstract algebra --- math-171/notes/Fibres and quotients#_definition, theorem _ the quotient group is a group|quotients]] to normal subgroups in the obvious way that we've been hinting at this whole time — the subgroups you can quotient out are exactly the normal subgroups.

##### _theorem:_ quotients are exactly those of normal subgroups

If $N \le G$, then the binary operation on the [[Abstract algebra --- math-171/notes/Fibres and quotients#_definition _ cosets|cosets]] of $N$ by
$$
(gN)(hN) = (gh)N
$$
...
In $D_{8}$, $\left< s \right> \trianglelefteq \left< s, r^2 \right> \trianglelefteq D_{8}$ (because each subgroup has [[Abstract algebra --- math-171/notes/Lagrange's theorem#_proposition _ index $2$ subgroups are normal|index 2]] in the next with orders 2, 4, and 8 respectively). However $\left< s \right>$ is not normal in $D_{8}$ since $rsr^{-1} = sr^{2} \not\in \left< s \right>$. Note that $sr^{2}$ is still in $\left< s, r^{2} \right>$ since [[Abstract algebra --- math-171/notes/Group automorphisms and automorphism groups#_proposition _ inner automorphisms are actually automorphisms|inner automorphisms]] of $D_{8}$ fix $\left< s, r^{2} \right>$. The problem is that even if a subset is invariant under a function, its elements may still get moved around within it.