...
A lot of the time we think of groups as [[Abstract algebra --- math-171/notes/Dihedral groups and generators|representing the symmetries of an object]]. However, a symmetry of a thing is a finicky thing and sometimes its better to think of the group as acting on the object instead, keeping compositions of actions separate from states of the object.
...
1) A [[Vector spaces#_definition _ vector space|vector space]] is an [[Abstract algebra --- math-171/notes/Groups, and why you should care#_definition _ abelian group, commutative group|abelian group]] $V$ acted on by the multiplicative group of its base [[Abstract algebra --- math-171/notes/Fields#_definition _ fields|field]] $\mathbb{F}^\times$, with the additional requirement that the group action distributes over the operation on the abelian group and $0v = 0$.
...
\end{split}
$$
5) If $G$ is graph, then $\operatorname{Aut} g$ [[Abstract algebra --- math-171/notes/Group automorphisms and automorphism groups#_example _ the automorphism group of a symmetric graph|acts on the set of vertices]] of $G$.
...
It's also notable that a group action induces structure on the group itself. Again, in [[Abstract algebra --- math-171/attachments/homework/hw 7/hw 7.pdf#page=1|the homework]] we verify that the subsets below really are [[Abstract algebra --- math-171/notes/Subgroups#_definition _ subgroup, $ le$|subgroups]].
...
The kernel of a group action $G \circlearrowright X$ is $\ker (G \circlearrowright X)$, the set of all $g \in G$ that fix all $x \in X$ — all $g \in G$ such that $g \cdot x = x$ for any $x \in X$. The kernel is a [[Abstract algebra --- math-171/notes/Subgroups#_definition _ subgroup, $ le$|subgroup]].

We say a group action is effective if it has trivial kernel. Note that if $G \circlearrowright X$ has kernel $H$, then the [[Abstract algebra --- math-171/notes/Normal subgroups#Quotient groups|quotient]] $G / H \circlearrowright X$ acts on $X$ effectively.
...
There is an incredibly useful theorem relating the structure on $G$ (stabilisers) and the structure on $X$ (orbits). This allows us to understand the structure of finite groups through [[Abstract algebra --- math-171/notes/The class equation#_theorem _ the class equation|the class equation]] and [[Abstract algebra --- math-171/notes/Sylow's theorem|the Sylow theorems]], and it gives us a powerful tool to solve combinatorial problems in [[Abstract algebra --- math-171/notes/Burnside's lemma|Burnside's lemma]].
...
Since this is a bijection, the sizes of the sets are the same. The last equality is just [[Abstract algebra --- math-171/notes/Lagrange's theorem#_theorem _ Lagrange's theorem|Lagrange's theorem]].

### Permutations from group actions

An important perspective to have is understanding group actions as homomorphisms to the [[Abstract algebra --- math-171/notes/Group combinatorics and symmetric groups#_definition _ the symmetric group on $ Omega$|symmetric group]] of the set they act on.
...
Suppose $G$ acts on $X$. For each $g \in G$, $\sigma_{g}$ is a permutation of $X$ and $\varphi : g \to \sigma_{g}$ is a [[Abstract algebra --- math-171/notes/Group homomorphisms#_definition _ group homomorphisms|group homomorphism]] $G \to S_{X}$ ([[Abstract algebra --- math-171/notes/Group combinatorics and symmetric groups#_definition _ the symmetric group on $ Omega$|the symmetric group]] on $X$).
