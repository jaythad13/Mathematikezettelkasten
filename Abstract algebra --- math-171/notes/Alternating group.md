...
Remember how subgroups of index $2$ [[Abstract algebra --- math-171/notes/Lagrange's theorem#_proposition _ index $2$ subgroups are normal|are normal]]? Let's look at an example.

##### _example:_ the sign of a permutation

Consider $\sigma \in S_{n}$, Then we can write $\sigma$ as the [[Abstract algebra --- math-171/notes/Group combinatorics and symmetric groups#_proposition _ every permutation is the composition of transpositions|product of transpositions]]. This product is not unique — there is more than one way to do this. However, the number of transpositions is either always even or always odd. This motivates us to define
...
\end{document}
```
tells us that $S_{n} /A_{n} \cong \{ -1, 1 \}$, $S_{n} / A_{n}$ has order $2$, and thus, $\lvert S_{n} : A_{n} \rvert = 2$ (this is all by [[Abstract algebra --- math-171/notes/Group isomorphism theorems|the first isomorphism theorem]]).
