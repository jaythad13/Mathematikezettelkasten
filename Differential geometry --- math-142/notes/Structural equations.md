...
How are the dual $1$-forms related to the connection forms? We should expect there to be a "dual version" of the [[Connection forms#_theorem, definition _ the connection equations, the connection forms|connection equations]].
...
[[Connection forms#_example _ connections on the cylindrical frame field|We have already computed]] that the connection forms are all $0$ except for $\omega_{12} = d\vartheta$. We claim that we also have $d \omega_{12} = 0$ and thus, $d\omega_{ij}$ is always $0$.
