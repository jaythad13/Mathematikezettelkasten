...
\begin{solution}
	We can prove this by induction on $n$ for $\bvec x = (x_{1}, \dots, x_{n}) \in \bb R^n$. For $n = 1$, we have
	\[
...
\begin{solution}
	...
	\]
	Moreover, since it is the union of finitely many open sets, it is a bounded set. Thus, its closure is a closed and bounded, and therefore compact subset $D \subset U$ and it is $D^\circ$, the open interior that contains $C$.
\end{solution}
...
\begin{solution}
	\begin{enumerate}
		...
		\item Choose $\delta = v_{2}/v_{1}$. Then, for any $\varepsilon > 0$, for all $\lambda$ with $\abs{\lambda - 0} < \delta$ we have $\abs{g_{\bvec v}(\lambda) - g_{\bvec v}(0)} = 0 < \varepsilon$. Thus, $g_{\bvec v}$ is continuous at $0$.

			However, for any $0 < \delta$, there are always points $(x, y) = (\delta/\sqrt{2}, \delta/2 \sqrt{2})$ with $\norm{(x, y) - \bvec 0} = \delta \sqrt{5/8} < \delta$ and $x^2 = \delta/2 > \delta/2 \sqrt{2} = y$. These points $(x, y)$ are in $A$ and give $f(x,y) = 1$.
