...
It turns out that this principle holds true for continuity as well — continuous functions are equivalent to continuous component functions.

##### _proposition:_ the projection principle holds for continuity

A function $f : \mathbb{R}^n \supset A \to \mathbb{R}^m$ is continuous if and only if each of its $m$ component functions $f_{1}, \dots, f_{m} : A \to \mathbb{R}$ is continuous.

###### _proof:_

[[Calculus --- spivak/attachments/exercises/Calculus on Manifolds/ex 1/ex 1.pdf#page=11|See the exercises]].
