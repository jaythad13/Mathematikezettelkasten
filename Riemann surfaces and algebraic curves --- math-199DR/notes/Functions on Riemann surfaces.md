...
Moreover, if we have open sets $V \subseteq W \subseteq X$, we have a map $\mathcal{O}_{X}(W) \to \mathcal{O}_{X}(V)$ that sends a function on $W$ to its restriction to $V$ (we want this to be an injection and [[Complex analysis --- math-135/notes/Cauchy integral formula#_corollary _ unique analytic continuation|analytic continuation]] will do that for us soon). To be exact, this is a [[Algebraic geometry --- rising-sea/notes/Functors|contravariant functor]] from the poset category on the topology of $X$ (with inclusions being the morphisms) to the category of [[Abstract algebra --- math-171/notes/Rings#_definition _ commutative ring|commutative rings]].
