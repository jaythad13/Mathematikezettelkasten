...
\begin{solution}
	Suppose $V, q$ is anisotropic but has isotropic base change $V', q'$ to $\mathbb{F}(\sqrt{d})$. Let $V, q \cong \left < a_{1}, \dots, a_{n} \right >$ (over $\mathbb{F}$), and thus, $V', q' \cong \left < a_{1}, \dots, a_{n} \right >$ (over $\mathbb{F}(\sqrt{d})$) as well.
...
\begin{solution}
	\begin{enumerate}
		...
		\item We assumed that $Sv = v - Tv$ is non-zero for anisotropic $v$. Thus, each $v \in \ker S$ is isotropic. It follows that for any $v, w \in \ker S$ we have $\beta(v, w) = 0$.

		\item Suppose $v \in \ker S$. That is, $Tv = v$. Then, for each $w \in V$,
			\begin{align*}
				2 \beta(v, Sw) & = q(v + Sw) - q(v) - q(Sw) \\
					       & = q(v + Sw) \\
			\end{align*}
			Consider

			Suppose $v \in (\operatorname{img} S)^{\perp}$. Then $\beta(v, Sw) = 0$ for each $w \in V$. In particular
			\begin{align*}
				0 & = 2 \beta(v, Sv) \\
				  & = q(v + Sv) - q(v) - q(Sv) \\
				  & = q(v + Sv) - q(v).
			...
			Since $\operatorname{img} S \subseteq (\operatorname{img} S)^{\perp} = \ker S$, we have $S^{2} = 0$.
	\end{enumerate}
\end{solution}
