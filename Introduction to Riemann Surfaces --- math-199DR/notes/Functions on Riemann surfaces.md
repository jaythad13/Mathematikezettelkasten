...
This definition is actually overkill. We could just check holomorphicity on a cover of $W$.
...
$f$ has a removable singularity at $p$ if there is a chart $(\phi, U)$ with $p \in U$ and $f \circ \phi ^{-1}$ has a  [[Complex Analysis --- math-135/notes/Meromorphic functions and singularities#_definition _ pole|pole]] at $p$.
...
By the same proof as for holomorphic functions, we can go in the opposite direction to show that all of these definitions don't depend on the choice of chart.

##### _definition:_ meromorphic

$f : X \to \mathbb{C}$ is meromorphic at $p \in X$ if it has, at worst, a pole at $p$.

$f$ is meromorphic on the open set $W \subseteq X$ if it is meromorphic at each point of $W$.
...
A corollary of the maximum modulus theorem that fails over $\mathbb{C}$ can be applied to compact Riemann surfaces. This is a similar result to [[Complex Analysis --- math-135/notes/Cauchy integral formula#_theorem _ Liouville's theorem|Liouville's theorem]], the hypothesis of which essentially forces $\mathbb{C}$ to behave as if it were compact.
...
Since $f$ is holomorphic, it is [[Mathematical Analysis I --- math-131/notes/Continuity#_definition _ continuity|continuous]], and thus, so is $\lvert f \rvert$. Since $X$ is compact, $\lvert f \rvert$ [[Mathematical Analysis I --- math-131/notes/Continuity#_corollary _ the image of a compact set has a maximum and minimum|achieves a maximum]], and thus, must be constant.


### Sheaves and stalks — an aside for algebraic geometry
...
Moreover, if we have open sets $V \subseteq W \subseteq X$, we have a map $\mathcal{O}_{X}(W) \to \mathcal{O}_{X}(V)$ that sends a function on $W$ to its restriction to $V$ (we want this to be an injection and [[Complex Analysis --- math-135/notes/Cauchy integral formula#_corollary _ unique analytic continuation|analytic continuation]] will do that for us soon). To be exact, this is a [[Algebraic geometry --- rising-sea/notes/Functors and natural transformations|contravariant functor]] from the poset category on the topology of $X$ (with inclusions being the morphisms) to the category of [[Abstract Algebra I --- math-171/notes/Rings#_definition _ commutative ring|commutative rings]].
...
The interesting case of functions is those that are holomorphic or meromorphic on the whole surface. This leads us to distinguish the global sections —
...
The projective line $\mathbb{C} \mathbb{P}^1$ is just the same thing as the Riemann sphere. Instead we write every point as the ratio of two numbers (upto scaling both numbers by the same thing). In particular, we write $\infty = (1 : 0)$. Thus, in these projective coordinates, meromorphic functions on $\mathbb{C} \mathbb{P}^1$ are exactly the ratios of homogeneous polynomials of the same degree.

This is much nicer because we can write zeroes and poles at $\infty$ as zeroes or poles of rational functions with homogeneous numerator and denominator of the same degree. That is, a meromorphic function $f$ has
$$
