...
We want to show that there is a neighbourhood of $y$ (say $V$) for which all pre-images of points $q \in V$ come from the discs around the $x_{i}$. For any sequence of points $q_{n} \to y$ in $Y$, we can create a sequence of pre-images $p_{n}$ in $X$ and, since $X$ [[Mathematical Analysis I --- math-131/notes/Compactness#_theorem _ Bolzano-Weierstrass theorem|is compact, a convergent subsequence]] $p_{n_{k}} \to x$. Since $\pi$ is continuous, $\pi(x) = y$, and thus $x$ is a pre-image of $y$. That is, arbitrarily close to $y$ points will always have pre-images in some disc surrounding $X$.
...
This allows us to prove what we already know to be true in the special cases of [[Introduction to Riemann Surfaces --- math-199DR/notes/Functions on Riemann surfaces#_example _ meromorphic functions on the Riemann sphere and projective line|the Riemann sphere]] and [[Introduction to Riemann Surfaces --- math-199DR/notes/Holomorphisms of Riemann surfaces#_proposition _ meromorphic functions on a torus $ mathbb{C} / Lambda$ have zero total order|complex tori]].
...
Since $2g - 2$ [[Introduction to Riemann Surfaces --- math-199DR/notes/Topological invariants of a surface#_theorem _ Euler characteristic and genus|is the negation of the Euler characteristic]], the sign inside the sum is flipped and we get our desired formula.
