...
Let $U \subseteq \mathbb{R}^n$ be open. If $v : U \to \mathbb{R}^n$ is a locally Lipschitz continuous vector field, then for each $x_{0} \in U$ there is a function $x : [0, \varepsilon) \to \mathbb{R}^n$ such that $x(0) = x_{0}$ and $x'(t) = v(x(t))$for each $t$. 

Further, $x$ depends continuously on $x_{0}$.
...
Let $X$ be the metric space of continuous functions $[0, \delta / 4 \lambda] \to U$ such that $\lVert x(0) - x_{0} \rVert \leq \delta / 4$ always and $\lVert x(t) - x_{0} \rVert \leq \delta / 2$ with the supremum metric. Let $\Gamma : X \times Y \to X$ be given by
$$
\Gamma(x, x_{0})(t) = x_{0} + \int_{0}^t v(x(s)) \, ds.
$$
Note $\Gamma(u, x_{0})$ really does lie in $X$. It's clear that $\Gamma(x, x_{0})(0) = x$ which is within $\delta / 4$ of $x_{0}$. Further
$$
\begin{align}
\lVert \Gamma(x, x_{0})(t) - x_{0} \rVert  & \left \lVert x_{0} +  \int_{0}^t v(x(s))\, ds - x \right\rVert  \\
  & \leq \lVert x - x_{0} \rVert + \int_{0}^t \lVert v(x(s)) \rVert \, ds  \\
 & \leq \delta / 4 + \lambda t \\
 & \leq \delta / 4 + \lambda \delta / 4 \lambda \\
 & \leq \delta / 2
\end{align}
$$
so $\Gamma(x, x_{0})$ is always within $\delta / 2$ of $x_{0}$.

Now we show each $x \mapsto \Gamma(x, x_{0})$ is a contraction.
$$
\begin{align}
d(\Gamma)(x_{1}, x_{0}), \Gamma(x_{2}, x_{0})) & = \max_{t}  \left\lVert  \int_{0}^t v(x_{1}(s)) \, ds -  \int_{0}^t v(x_{2}(s)) \, ds   \right\rVert \\
 & \leq \max_{t} \int _{0}^t \lVert v(x_{1}(s)) - v(x_{2})(s) \rVert  \, ds \\
 & \leq \max_{t} \int_{0}^t \lambda \lVert x_{1}(s) - x_{2}(s) \rVert \, ds \\
 & \leq \frac{\delta}{4 \lambda} \lambda d(x_{1}, x_{2}) \\
 & = \frac{\delta}{4} d(x_{1}, x_{2}).
\end{align}
$$
But we chose $\delta$ small enough so that $\delta / 4 < 1$.

Thus, for each $x \in B(x_{0}, \delta / 4)$ [[Analysis --- math-131/notes/Cauchy sequences and completeness#_theorem _ contraction mapping principle, or the Banach fixed point theorem|there is a unique fixed point]] of $x \mapsto \Gamma(x, x_{0})$. Call this fixed point $x$. Then $x$ satisfies $x(t) = x_{0} + \int_{0}^t v(x(s)) \, ds$. But that is just a solution of the differential equation — we have $x(0) = x_{0}$ and $x'(t) = v(x(t))$.
