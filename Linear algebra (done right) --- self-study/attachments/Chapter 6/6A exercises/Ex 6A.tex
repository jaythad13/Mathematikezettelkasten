...
\begin{solution}
	...
	\[
		\Delta ((1 - \norm{x}^2) s) = 0
	\]
	Thus, \( (1 - \norm{x}^2) s \) is harmonic. Since $(1 - \norm{x}^2) s$ is harmonic and $0$ everywhere on the unit sphere, $(1 - \norm{x}^2)s$ must be $0$ everywhere. Thus $s$ must be zero everywhere. That is, if $Ts = 0$, $s = 0$.\\\\
	This gives us that $T$ is injective and thus, surjective. Then $Tr = - \Delta q$ for some $r$, giving us the $p$ we want as
	\[
		p = (1 - \norm{x}^2)r + q
	\]
	for that $r$.
