...
\begin{solution}
	\[
		\boxed{
			q = \frac{\sin \pi}{\pi} + \frac{\cos{\pi} + \pi \sin{\pi} - 1}{\pi^2} \, x + \frac{2 \pi \cos{\pi} + (\pi - 2) \sin{\pi}}{\pi^3} \, x^2.
		}
	\] \\
	...
	\[
		\begin{split}
			q & = \overline{ \left ( \int_0^1 1 \cos{\pi x} \, dx \right )} \, e_1 + \overline{\left ( \int_0^1 x \cos{\pi x} \, dx \right )} \, e_2 + \overline{\left ( \int_0^1 x^2 \cos{\pi x} \, dx \right )} \, e_3 \\
			  & = \frac{\sin \pi}{\pi} + \frac{\cos{\pi} + \pi \sin{\pi} - 1}{\pi^2} \, x + \frac{2 \pi \cos{\pi} + (\pi - 2) \sin{\pi}}{\pi^3} \, x^2
		\end{split}
	\]
\end{solution}
...
\begin{problem}[16.]
	Suppose $V$ is a finite-dimensional vector space over $\mathbb C$, $T \in \mathcal L(V)$, all the eigenvalues of $T$ have absolute value less than $1$, and $\varepsilon > 0$. Prove that there exists a positive integer $m$ such that $\Vert T^m \Vert \le \varepsilon \Vert v \Vert$ for every $v \in V$.
\end{problem}
...
\begin{problem}[17.]
	...
	\begin{enumerate}
		\item
			Show that if $\mathbb F = \mathbb R$, then $\Phi$ is a linear map from $V$ to $V'$.
		\item
			Show that if $\mathbb F = \mathbb C$, and $V \neq \{ 0 \}$, then $\Phi$ is not a linear map.
		\item
			Show that $\Phi$ is injective.
		\item
			Suppose $\mathbb F = \mathbb R$ and $V$ is finite-dimensional. Use parts $(a)$ and $(c)$ and a dimension-counting argument to show that $\Phi$ is an isomorphism from $V$ onto $V'$.
	\end{enumerate}
\end{problem}
