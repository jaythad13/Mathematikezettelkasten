...
Let $X$ be a [[Mathematical Analysis I --- math-131/notes/Cauchy sequences#Completeness|complete metric space]] and let $f$ be a contraction (a map such that $d(f(x), f(y)) < c \, d(x, y)$ for some positive $c < 1$). Then $f$ has a unique fixed point.

###### _proof sketch:_

Iteratively apply $f$ to $x$, and see that this forms a [[Mathematical Analysis I --- math-131/notes/Cauchy sequences|Cauchy sequence]] by the contraction condition. Since $X$ is complete, it converges to a point in $X$. This point is the fixed point. There can't be another one since the distance between them would have to contract.
...
The disc must be closed since otherwise you could contract towards a point not included on the boundary, which would. To prove this in general requires a lot of machinery, typically from homology theory, or Sperner's lemma. However, it's actually doable in the case $n \le 2$ (with just the machinery of the [[Topics in Geometry and Topology --- math-145/notes/Continuous and differentiable winding number#_theorem _ the continuous winding number theorem|continuous winding number theorem]]).
...
By the [[Mathematical Analysis I --- math-131/notes/Continuity#_corollary _ intermediate value theorem|intermediate value theorem]] we can see that $g$ defined by $g(x) = f(x) - x$ has $g(-1) \ge -1 - (-1) = 0$ and $g(1) \le 1 - 1 = 0$, so $g$ must have a zero. That is, there is some $x$ where $f(x) = x$.
