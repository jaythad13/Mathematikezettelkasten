...
Note that his is a weakening of the notion of an [[Abstract Algebra I --- math-171/notes/Exact sequences#_definition _ exact sequence|exact sequences]] — we only require $\operatorname{img} \partial_{k} \subseteq \ker \partial_{k - 1}$ instead of equality. The degree to which it is not exact is measured by homology.
...
Given two [[Abstract Algebra I --- math-171/notes/Exact sequences#_example _ short exact sequence|short exact sequences]], a map between them is three maps such that the following diagram commutes.
```tikz
\usepackage{tikz-cd}
\usepackage{amsfonts}
\begin{document}
