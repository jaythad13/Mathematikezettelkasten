...
\begin{solution}
	\begin{enumerate}
		\item If $m/n = p/q$, we have $mq = pn = k$. Thus, we can write $(b^{m})^{1/n}$ and $(b^{p})^{1/q}$ as $k$th roots of $b^{mp}$ ---
			...
			\begin{lemma}
				For any $b \in \mathbb{R}$ and $x, y \in \mathbb{N}$, $b^{1/xy} = (b^{1/x})^{1/y}$.
			\end{lemma}
		...
		\item First we will show that $b^{r}$ is an upper bound on $B(r)$. \\
			...
			Any element of $B(r)$ is of the form $b^{t}$ for some rational $t \le r$. We can write $b^{r} = b^{t} b^{r - t}$ where $r - t$ is some nonnegative rational. Since $b > 1$, $b^{r - t} \ge 1$, and thus, $b^{r} \ge b^{t}$. Thus, $b^{r}$ is an upper bound on $B(r)$. Now we will show that it is the least upper bound. \\
