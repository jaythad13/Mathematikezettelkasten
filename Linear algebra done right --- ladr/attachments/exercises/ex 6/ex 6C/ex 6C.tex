\documentclass[12 pt, letterpaper]{hmcpset}
\usepackage{hw-preamble}

\name{Jayden Thadani}
\class{Linear algebra done right}
\assignment{Exercises 6.C --- Orthogonal Complements and Minimisation Problems}
\duedate{}

\begin{document}

\begin{problem}[1]
	Suppose $v_{1}, \dots, v_{m} \in V$. Prove that
	\[
		\{ v_{1}, \dots, v_{m} \}^{\perp} = (\operatorname{span}(v_{1}, \dots, v_{m}))^\perp.
	\]
\end{problem}

\begin{solution}
	We will show this by double containment.

	Suppose  $u \in \{ v_{1}, \dots, v_{m} \}^{\perp}$. Then $\left < v_{j}, u \right > = 0$ for each $j \in \mathbb{N}_{m}$. Thus, by linearity in the first slot, for any $v = a_{1} v_{1} + \dots + a_m v_{m} \in (\operatorname{span}(v_{1}, \dots, v_{m}))^{\perp}$, we have $\left < v, u \right > = 0$. That is, $v \in (\operatorname{span}(v_{1}, \dots, v_{m}))^{\perp}$, and thus, $(\operatorname{span}(v_{1}, \dots, v_{m}))^{\perp} \subset \{ v_{1}, \dots, v_{m} \}^{\perp}$.

	Since $\{ v_{1}, \dots, v_{m} \} \subset \operatorname{span}(v_{1}, \dots, v_{m})$, obviously, we have $(\operatorname{span}(v_{1}, \dots, v_{m}))^{\perp} \subset \{ v_{1}, \dots, v_{m} \}^{\perp}$.
\end{solution}

\pagebreak

\begin{problem}[2]
	Suppose $U$ is a finite-dimensional subspace of $V$. Prove that $U^{\perp} = \{ 0 \}$ if and only if $U = V$.
\end{problem}

\begin{solution}
	If $U = V$ then $U^{\perp} = \{ 0 \}$. This is because the only vector $u \in U$ that gives $\left < v, u \right > = 0$ for all $v \in V$ is $u = 0$.

	Suppose $U^{\perp} = \{ 0 \}$. Then $U = (U^{\perp})^{\perp}$ and thus, $U = \{ 0 \}^{\perp}$. That is, $U = V$.
\end{solution}

\pagebreak

\begin{problem}[3]
	Suppose $U$ is a subspace of $V$ with the basis $u_{1}, \dots, u_{m}$ and
	\[
		u_{1}, \dots, u_{m}, w_{1}, \dots, w_{n}
	\]
	is a basis of $V$. Prove that if the Gram-Schmidt procedure is applied to the basis of $V$ above, producing a list $e_{1}, \dots, e_{m}, f_{1}, \dots, f_{n}$, then $e_{1}, \dots, e_{m}$ is an orthonormal basis of $U$ and $f_{1}, \dots, f_{n}$ is an orthonormal basis of $U^{\perp}$.
\end{problem}

\begin{solution}
	We know that the Gram-Schmidt procedure preserves spans. Thus, $e_{1}, \dots, e_{m}$ is a basis of $U$.
	
	By definition, $\left < e_{j}, f_{k} \right > = 0$ for all $j \in \mathbb{N}_{m}, k \in \mathbb{N}_{n}$. Then, by linearity, any $u \in U$ gives $\left < f_{k}, u \right > = \left < u, f_{k} \right >$ for all $k \in \mathbb{N}$. Then, again by linearity, any $w = b_{1} f_{1} + \dots + b_{n} f_{n} \in \operatorname{span}(f_{1}, \dots, f_{n})$ gives us $\left < u, w \right > = 0$ for all $u \in U$. Thus, $\operatorname{span}(f_{1}, \dots, f_{n}) \subset U^{\perp}$.

	For any $w \in U^{\perp}$, we can write $w = a_{1} e_{1} + \dots a_{m} e_{m} + b_{1} f_{1} + \dots + b_{n} f_{n}$. We must have $\left < e_{j}, w \right > = 0$ for all $j \in \mathbb{N}_{m}$, but $\left < e_{j}, w \right > = a_{j}$. Thus, $a_{j} = 0$ for all $j \in \mathbb{N}_{m}$ and $w \in \operatorname{span}(f_{1}, \dots, f_{n})$. That is, $U^{\perp} \subset \operatorname{span}(f_{1}, \dots, f_{n})$.

	Thus, by double containment, $U^{\perp} = \operatorname{span}(f_{1}, \dots, f_{n})$ of which $f_{1}, \dots, f_{n}$ is obviously an orthonormal basis.
\end{solution}

\pagebreak

\begin{problem}[4]
	Suppose $U$ is the subspace of $\mathbb{R}^{4}$ defined by
	 \[
		U = \operatorname{span}((1, 2, 3, -4), (-5, 4, 3, 2)).
	\]
	Find an orthonormal basis of $U$ and an orthonormal basis of $U^{\perp}$.
\end{problem}

\begin{solution}
	Call the two vectors $u_{1}$ and $u_{2}$ respectively. We can extend $u_{1}, u_{2}$ to a basis of $\mathbb{R}^{4}$. Notice that $(1, 0, 0, 0)$ and $(0, 1, 0, 0)$ are not in $U$. Thus, $u_{1}, u_{2}, e_{1}, e_{2}$ forms a basis of $\mathbb{R}^{4}$. \\

	Applying Gram-Schmidt to this gives us the orthonormal basis
	\[
		f_{1}, f_{2}, f_{3}, f_{4} = \frac{(1, 2, 3, -4)}{\sqrt{30}}, \frac{(-77, 56, 39, 38)}{\sqrt{30}}, \frac{(1/401, 117, 60, 151)}{\sqrt{76190}}, \frac{(0, 9, -10, -3)}{\sqrt{190}}.
	\]
	$f_{1}, f_{2}$ is an orthonormal basis of $U$ and $f_{3}, f_{4}$ is an orthonormal basis of $U^{\perp}$.
\end{solution}

\pagebreak

\begin{problem}[5]
	Suppose $V$ is finite-dimensional and $U$ is a subspace of $V$. Show that $P_{U^{\perp}} = I - P_{U}$, where $I$ is the identity operator on $V$.
\end{problem}

\begin{solution}
	We know that we can uniquely decompose any $v \in V$ into $w + u$ with $w \in U^{\perp}$ and $u \in (U^{\perp})^{\perp} = U$. But then this is also the unique decomposition into $u + w$ with $u \in U$ and $w \in U^{\perp}$. Thus, $P_{U} v = u$ and
	\[
		\begin{split}
			P_{U^{\perp}} & = w \\
				      & = v - u \\
				      & = I v - P_{U} v \\
				      & = (I - P_{U}) v.
		\end{split}
	\]
	Thus, $P_{U^{\perp}} = I - P_{U}$.
\end{solution}

\pagebreak

\begin{problem}[6]
	Suppose $U$ and $W$ are finite-dimensional subspaces of $V$. Prove that $P_{U} P_{W} = 0$ if and only if $\left < u, w \right > = 0$ for all $u \in U$ and all $w \in W$.
\end{problem}

\begin{solution}
	Suppose $\left < u, w \right > = 0$ for all $u \in U$ and $w \in W$. Then, by definition, $W \subset U^{\perp}$ and $U \subset W^{\perp}$. Thus, $\operatorname{range} P_{W} = W \subset U^{\perp} = \operatorname{null} P_{U}$. Thus, $P_{U} P_{W} = 0$.

	Suppose $P_{U} P_{W} = 0$. Then we must have $\operatorname{range} P_{W} \subset \operatorname{null} P_{U}$. However, $\operatorname{range} P_{W}$ is just $W$ and $\operatorname{null} P_{U} = U^{\perp}$. Thus, $W \subset U^{\perp}$. We know that taking the orthogonal complement of both sides reverses the inclusion, so $U \subset W^{\perp}$ as well. Thus, $\left < u, w \right > = 0$ for all $u \in U$ and $w \in W$.
\end{solution}

\pagebreak

\begin{problem}[7]
	Suppose $V$ is finite-dimensional and $P \in \mathcal{L}(V)$ is such that $P^{2} = P$ and every vector in $\operatorname{null} P$ is orthogonal to every vector in $\operatorname{range} P$. Prove that there exists a subspace $U$ of $V$ such that $P = P_{U}$.
\end{problem}

\begin{solution}
	Note that by rank-nullity we have $\dim \operatorname{null} P + \dim \operatorname{range} P = \dim V$. This, combined with the fact that $\operatorname{null} P \perp \operatorname{range} P$, gives us $\dim (\operatorname{null} P \oplus \operatorname{range} P) = \dim V$. Thus, we have $\operatorname{null} P \oplus \operatorname{range} P = V$.

	For any $v \in V$, write $v = u + w$ with $u \in \operatorname{range} P$ and $w \in \operatorname{null} P$. Then, we must have $Pv = Pu$. From here, it is sufficient to show that $Pu = u$ for $u \in \operatorname{range} P$ which would give us $P = P_{U}$ for $U = \operatorname{range} P$.

	If $u \in \operatorname{range} P$, then we can write $u = Pv$ for some $v \in V$ and thus, for $v = u' + w'$ as the decomposition into $\operatorname{range} P \oplus \operatorname{null} P$, we have $Pu' = u$ and $u' \in \operatorname{range} P$. Then since we know $P^{2} = P$, and thus,
	\[
		\begin{split}
			P^{2} u' & = P u' \\
			P u & = u.
		\end{split}
	\]
	Thus, $P$ is just the projection onto $\operatorname{range} P$.
\end{solution}

\pagebreak

\begin{problem}[8]
	Suppose $V$ is finite-dimensional and $P \in \mathcal{L}(V)$ is such that $P^{2} = P$ and
	\[
		\lVert Pv \rVert \le \lVert v \rVert
	\]
	for every $v \in V$. Prove that there exists a subspace $U$ of $V$ such that $P = P_{U}$.
\end{problem}

\begin{solution}
	We will show that $\operatorname{null} P \perp \operatorname{range} P$ and let the result follow from the previous exercise. Suppose $u = Pv \in \operatorname{range} P$ and $w \in \operatorname{null} P$. Then note that since $P^{2} v = Pv$, we have $Pu = u$. Thus, for any $a \in \mathbb{F}$
	\[
		\begin{split}
			P(u + a w) & = Pu + a Pw \\
				   & = u + 0 \\
				   & = u.
		\end{split}
	\]
	That is, $u = P(u + aw)$. Thus, their norms are equal too, giving us
	\[
		\begin{split}
			\lVert u \rVert^{2} & = \lVert P(u + aw) \rVert^{2} \\
					    & = \lVert u + aw \rVert^{2}
		\end{split}
	\]
	also for all $a \in \mathbb{F}$. By exercise 6A.6, we have that $\left < u, w \right > = 0$. That is, $\operatorname{null} P \perp \operatorname{range} P$ as desired.
\end{solution}

\pagebreak

\begin{problem}[9]
	Suppose $T \in \mathcal{L}(V)$ and $U$ is a finite-dimensional subspace of $V$. Prove that $U$ is invariant under $T$ if and only if $P_{U} T P_{U} = T P_{U}$.
\end{problem}

\begin{solution}
	First we will show that if $U$ is invariant under $T$, then the equality follows. Suppose $U$ is invariant under $T$. If $U$ is invariant under $T$, then for $u \in U$, $Tu \in U$. Thus, for any $v \in V$, with orthogonal decomposition $v = u + w$ into $U$ and $U^{\perp}$ we have $T P_{U} v = T u$. Since $Tu \in U$, $P_{U} T u = Tu$. That is, $P_{U} T P_{U} = T P_{U}$.

	Now we will show that if the equality holds, $U$ must be invariant under $T$. Suppose $P_{U} T P_{U} = T P_{U}$. For any $u \in U$, we of course have $P_{U} u = u$, and thus, $Tu = T P_{U} u$. Thus, we know that
	\[
		\begin{split}
			Tu & = T P_{U} u \\
			   & = P_{U} T P_{U} u \in U.
		\end{split}
	\]
	That is, $U$ is invariant under $T$.
\end{solution}

\pagebreak

\begin{problem}[10]
	Suppose $V$ is finite-dimensional, $\mathrm{T} \in \mathcal{L}(V)$, and $U$ is a subspace of $V$. Prove that $U$ and $U^{\perp}$ are both invariant under $T$ if and only if $P_{U} T = T P_{U}$.
\end{problem}

\begin{solution}
	First we will show the forward direction. Suppose $U$ and $U^{\perp}$ are invariant under $T$. Then for $u \in U$, $P_{U} u = u$ and $T u \in U$, giving $P_{U} T u = Tu$. Thus, $P_{U} T u = T u = T P_{U} u$ for any $u \in U$. If $w \in U^{\perp}$, then $P_{U} w = 0$ and $T w \in W$, giving $P_{U} T w = 0$. Thus, $P_{U} T w = 0 = T P_{U} w$ for any $w \in W$. Since $P_{U} T = T P_{U}$ on $U$ and $U^{\perp}$, and $V = U \oplus U^{\perp}$, the equality holds on all of $V$ as well.

	Now we suppose $P_{U} T = T P_{U}$. Then for any $u \in U$, since $u = P_{U} u$, we can write $Tu$ as $T P_{U} u = P_{U} T u \in U$. Thus, $U$ is invariant under $T$. For any $w \in U^{\perp}$, $P_{U} w = 0$, and thus, $T P_{U} w = 0$. Thus,  $P T w = 0$. That is $T w \in \operatorname{null} P_{U} = U^{\perp}$. Thus, $U^{\perp}$ is invariant under $T$ too.
\end{solution}

\pagebreak

\begin{problem}[11]
	In $\mathbb{R}^{4}$ let
	 \[
		U = \operatorname{span}((1, 1, 0, 0), (1, 1, 1, 2)).
	\]
	Find $u \in U$ such that $\lVert u - (1, 2, 3, 4) \rVert$ is as small as possible.
\end{problem}

\begin{solution}
	First, we find an orthonormal basis of $U$, by applying Gram-Schmidt to $(1, 1, 0, 0), (1, 1, 1, 2)$. The first basis vector is $(1, 1, 0, 0)$ normalised to $e_{1} = (1, 1, 0, 0)/\sqrt{2}$. Then the second basis vector is
	\[
		\begin{split}
			(1, 1, 1, 2) - \frac{(1, 1, 1, 2) \cdot (1, 1, 0, 0)}{\sqrt{2}} \frac{(1, 1, 0, 0)}{\sqrt{2}} & = (1, 1, 1, 2) - \frac{2}{2} (1, 1, 0, 0) \\
														      & = (0, 0, 1, 2)
		\end{split}
	\]
	normalised to $e_{2} = (0, 0, 1, 2) / \sqrt{5}$.

	The desired $u$ is then just the projection of  $(1, 2, 3, 4)$ onto $U$ ---
	\[
		\begin{split}
			u & = \frac{(1, 2, 3, 4) \cdot (1, 1, 0, 0)}{\sqrt{2}} \frac{(1, 1, 0, 0)}{\sqrt{2}} + \frac{(1, 2, 3, 4) \cdot (0, 0, 1, 2)}{\sqrt{5}} \frac{(0, 0, 1, 2)}{\sqrt{5}} \\
			  & = \frac{3}{2} (1, 1, 0, 0) + \frac{11}{5} (0, 0, 1, 2) \\
			  & = \left ( \frac{3}{2}, \frac{3}{2}, \frac{11}{5}, \frac{22}{5} \right ).
		\end{split}
	\]
\end{solution}

\pagebreak

\begin{problem}[12]
	Find $p \in \mathcal{P}_{3}(\mathbb{R})$ such that $p(0) = 0$, $p'(0) = 0$, and
	\[
		\int_{0}^{1} \lvert 2 + 3x - p(x) \rvert^{2} \, dx 
	\]
	is as small as possible.
\end{problem}

\pagebreak

\begin{problem}[13]
	Find $p \in \mathcal{P}_{5}(\mathbb{R})$ that makes
	\[
		\int_{- \pi}^{\pi} \lvert \sin{x} - p(x) \rvert^{2} \, dx 
	\]
	as small as possible.
\end{problem}

\pagebreak

\begin{problem}[14]
	Suppose $\mathcal{C}([-1, 1])$ is the vector space of continuous real-valued functions on the interval $[-1, 1]$ with inner product given by
	\[
		\left < f, g \right > = \int_{-1}^{1} f(x) g(x) \, dx 
	\]
	for $f, g \in \mathcal{C}([-1, 1])$. Let $U$ be the subspace of $\mathcal{C}([-1, 1])$ defined by
	 \[
		U = \{ f \in \mathcal{C}([-1, 1]) \mid f(0) = 0 \}.
	\]
	\begin{enumerate}
		\item Show that $U^{\perp} = \{ 0 \}$.
		\item Show that 6.47 and 6.51 do not hold without the finite-dimensional hypothesis.
	\end{enumerate}
\end{problem}

\begin{solution}
	\begin{enumerate}
		\item We can use the same trick from Exercises 6B, 15.

			Suppose $g \in U^{\perp}$. Then $f$ defined by $f(x) = x^{2} g(x)$ gives $f \in U$. Since $g \in U^{\perp}$, we must have $\left < f, g \right > = 0$, but we also know $\left < f, g \right > = \lVert x^{2} g \rVert$. Then $x^{2} g$ has norm zero and must be the zero function. Then since $x = 0$ is the only zero of $x^{2}$, $g$ must be zero everywhere on $\mathbb{R} \setminus \{ 0 \}$, and then, by continuity, at $0$ as well. That is, $g = 0$.
		\item 6.47 tells us that an inner-product space $V$ should decompose as $U \oplus U^{\perp}$ for a finite-dimensional subspace $U$, but here $U + U^{\perp} = U \subsetneq V$. 6.51 says that $U = (U^{\perp})^{\perp}$. Here $U^{\perp} = \{ 0 \}$ and thus  $(U^{\perp})^{\perp} = V$ but  $U \subsetneq V$. These show that the theorem doesn't hold in the infinite-dimensional case.
	\end{enumerate}
\end{solution}

\end{document}
