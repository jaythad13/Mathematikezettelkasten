...
\begin{solution}~
	...
	Since all the terms on the left hand side are non-negative, we can write
        ...
        Since $V$ is a field over $\bb{R}$ or $\bb{C}$, both of which are complete, for all $p$, $\set{a_{n, p}}$ converges to some $A_p$. That is, for any \( \varepsilon' > 0 \), there is some \( N \in \bb{N} \) such that for all \( n > N \), \( \abs{A_p - a_{n, p}} < \varepsilon' \). We will show that the sequence of vectors converges to \( A_1 e_1 + \ldots + A_m e_m \). \\
