...
\begin{problem}[6]
	Find an orthonormal basis of $\curly{P}_2(\bb{R})$ (with inner product as in exercise 5) such that the differentiation operator (the operator that takes $p$ to $p'$) has an upper-triangular matrix with respect to this basis.
\end{problem}
...
\begin{solution}
	...
	\]
	Since we have $\langle v', u \rangle_i = 0$ we have
	\[
		\begin{split}
	...
	\]
	Since $c_u \in \mathbb{R}$, we can rewrite this as 
	\[
...
\begin{solution}
	Consider the functional $\varphi \in V^{\vee}$ given by $\varphi(v_{j}) = 1$. By the Riesz representation theorem, there exists a $w \in V$ such that $\left < v, w \right > = \varphi(v)$ for all $v \in V$ (which, of course, includes all the $v_{j}$). Since $\varphi(v_{j}) \in \mathbb{R}$, for all $j$, we can swap the slots. That is, $\left < w, v_{j} \right > = \varphi(v_{j})$ for all $j$. Thus, $\left < w, v_{j} \right > = 1 > 0$ for all $j$.
\end{solution}
...
\begin{solution}
	Suppose that $a_{1}, \dots, a_{n}$ are not all $0$. Notice that we have
	\[
		\begin{split}
			\left \lVert \sum_{i = 1}^{n} a_{i} e_{i} \right \rVert - \left \lVert \sum_{i = 1}^{n} a_{i} v_{i} \right \rVert & \le \left ( \sum_{i = 1}^{n} \lVert a_{i} e_{i} \rVert \right ) - \left ( \sum_{i = 1}^{n} \lVert a_{i} v_{i} \rVert \right ) \\
																	  & \le \sum_{i = 1}^{n} \lvert a_{i} \rvert \lVert e_{i} - v_{i} \rVert \\
																	  & < \frac{1}{\sqrt{n}} \sum_{i = 1}^{n} \lvert a_{i} \rvert \\
																	  & = \frac{1}{\sqrt n} \left \lVert \sum_{i = 1}^{n} a_{i} e_{i} \right \rVert.
		\end{split}
	\]
	Thus,
	\[
		\begin{split}
			\left \lVert \sum_{i = 1}^{n} a_{i} v_{i} \right \rVert & > \left ( 1 - \frac{1}{\sqrt n} \right ) \left \lVert \sum_{i = 1}^{n} a_{i} e_{i} \right \rVert \\
										& > 0.
		\end{split}
	\]
	Thus, any non-zero linear combination of $v_{1}, \dots, v_{n}$ is non-zero. That is, $v_{1}, \cdots, v_{n}$ is linearly independent. Since it is a list of the right length, it is a basis.
\end{solution}
...
\begin{solution}
	Suppose, by way of contradiction, that such a $g$ existed. Then, for $f \in \mathcal{C}_{\mathbb{R}}([-1, 1])$ defined by $f(x) = x^{2} g(x)$, we must have
	\[
	...
	\[
		\int_{-1}^{1} (x g(x))^{2} \, dx = 0
	\]
	which is a contradiction since $x g \neq 0$ and thus, its norm should also be non-zero. (If $x g = 0$, then $g = 0$ and thus, $g$ would not have the desired property for functions $h$ with $h(0) \neq 0$).
\end{solution}
...
\begin{solution}
	...
	However, by scaling our basis, we can make these terms arbitrarily small. Choose some positive real $\delta < 1$, and then set $f_{j} = \delta^{n + 1 - j} e_{j}$ for all $j \in \mathbb{N}_{n}$. That is, set $f_{n} = e_{n}$, $f_{n - 1} = \delta e_{n - 1}$, and so on. Let $B$ be the upper-triangular matrix of $T$ with respect to $f_{1}, \dots, f_{n}$. Then $B_{j, k} = \delta^{k - j} A_{j, k}$. Moreover, for any $v = b_{1} f_{1} + \cdots + b_{n} f_{n} = a_{1} e_{1} + \cdots + a_{n} e_{n}$ we have $b_{k} = a_{k}/\delta^{n + 1 - k}$.

	This means that, for any such $v$, we have
	\[
		\begin{split}
			Tv & = \sum_{j = 1}^{n} \left ( \sum_{k = j}^{n} b_{k} B_{j, k} \right ) f_{j} \\
			   & = \sum_{j = 1}^{n} \left ( \delta^{n + 1 - j} \sum_{k = j}^{n} b_{k} B_{j, k} \right ) e_{j}.
		\end{split}
	\]
	However, we also have the bound
	\[
		\begin{split}
			\left \lvert \delta^{n + 1 - j} \sum_{k = j}^{n} b_{k} B_{j, k} \right \rvert & \le \delta^{n + 1 - j} \left \lvert b_{j} B_{j, j} \right \rvert + \delta^{n + 1 - j} \sum_{k = j + 1}^{n} \left \lvert b_{k} B_{j, k} \right \rvert \\
										   & \le \lvert a_{j} \rvert \lvert A_{j, j} \rvert + \delta \sum_{k = j + 1}^{n} \delta^{k - j - 1} \lvert a_{j} A_{j, k} \rvert \\
										   & \le \lvert a_{j} \rvert \lvert A_{j, j} \rvert + \delta \sum_{k = j + 1}^{n} \lvert a_{j} A_{j, k} \rvert \\
										   & \le \lambda_{j} \lvert a_{j} \rvert
		\end{split}
	\]
	for any $\lambda_{j}$ with $\lvert A_{j, j} \rvert < \lambda_{j} < 1$. We get this by picking $\delta$ so that the last term is arbitrarily small. Finally, we can choose $\lambda = \max_{j \in \mathbb{N}_{n}} \lambda_{j}$ and bound $\lVert Tv \rVert$. Specifically, 
	\[
		\begin{split}
			\lVert Tv \rVert^{2} & = \left \lVert \sum_{j = 1}^{n} \left ( \sum_{k = j}^{n} b_{k} B_{j, k} \right ) f_{j} \right \rVert^{2} \\
					 & = \sum_{j = 1}^{n} \left \lvert \delta^{n + 1 - j} \sum_{k = j}^{n} b_{k} B_{j, k} \right \rvert^{2} \\
					 & \le \lambda^{2} \sum_{j = 1}^{n} \lvert a_{j} \rvert^{2} \\
					 & = \lambda^{2} \lVert v \rVert^{2}.
		\end{split}
	\]
	That is, $\lVert Tv \rVert \le \lambda \lVert v \rVert$ for some $\lambda < 1$. Then, for any positive integer $m \ge \log_{\lambda} \varepsilon$, we have $\lVert T^{m} v \rVert \le \varepsilon \lVert v \rVert$ for all $v \in V$.
\end{solution}
