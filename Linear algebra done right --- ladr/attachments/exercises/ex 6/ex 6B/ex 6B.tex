...
\begin{solution}~
	...
	However, by scaling our basis, we can make these terms arbitrarily small. Choose some positive $\delta \in \mathbb{R}$, and then set $f_{n + 1 - j} = \delta^{j} e_{n + 1 - j}$ for all $j \in \mathbb{N}_{n}$. Let $B$ be the upper-triangular matrix of $T$ with respect to $f_{1}, \dots, f_{n}$. Then $B_{j, k} = \delta^{k - j} A_{j, k}$. Moreover, for any $v = b_{1} f_{1} + \cdots + b_{n} f_{n} = a_{1} e_{1} + \cdots + a_{n} e_{n}$ we have $b_{k} = a_{k}/\delta^{n + 1 - k}$ \\
	...
	Finally, we can choose $\lambda = \max_{j \in \mathbb{N}_{n}} \lambda_{j}$ and bound $\lVert Tv \rVert$. Specifically, 
	\[
		\begin{split}
			\lVert Tv \rVert^{2} & = \left \lVert \sum_{j = 1}^{n} \left ( \sum_{k = j}^{n} b_{k} B_{j, k} \right ) e_{j} \right \rVert^{2} \\
					 & = \sum_{j = 1}^{n} \left \lvert \sum_{k = j}^{n} b_{k} B_{j, k} \right \rvert^{2} \\
					 & \le \lambda^{2} \sum_{j = 1}^{n} \lvert b_{j} \rvert^{2} \\
