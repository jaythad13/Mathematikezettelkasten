...
\begin{solution}
	We can write the partial sums of $\sum \sqrt{a_{n}} / n$ as the dot product of two vectors ---
	\[
		s_{n} = \frac{\sqrt{a_{1}}}{1} + \dots + \frac{\sqrt{a_{n}}}{n} = (\sqrt{a_{1}}, \dots, \sqrt{a_{n}}) \cdot \left ( 1, \frac{1}{2}, \dots, \frac{1}{n} \right ).
	\]

	Then by Cauchy-Schwartz, we have
	\[
		\sum_{k = 1}^{n} \frac{\sqrt{a_{k}}}{k} \le \sqrt{\sum_{k = 1}^{n} a_{k}} \sqrt{\sum_{k = 1}^{n} \frac{1}{k^{2}}}
	\]
	But since both of the series on the right are convergent, their partial sums are bounded. Thus, the partial sums of $\sum \sqrt{a_{n}} / n$ are bounded. Since the terms are positive, the sequence of partial sums converges, and thus, so does the series.
\end{solution}
