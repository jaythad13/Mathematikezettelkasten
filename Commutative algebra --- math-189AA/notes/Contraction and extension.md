...
We could try to do the same thing under pushing forward ideals, but it doesn't quite work. For example, $A \to A[x]$ has images of ideals that are not themselves ideals (any $\mathfrak{i} \subseteq A$ is not closed under scaling by $x$). We force this to work by instead considering the ideal generated by the image.
