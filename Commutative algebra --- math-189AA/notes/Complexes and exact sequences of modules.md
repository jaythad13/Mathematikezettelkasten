...
Exact sequences are useful! They tell you things!
...
##### _example:_ exact sequences and (finite) generation
...
\end{document}
```
is exact with $A$ generated by $X \subseteq A$ and $C$ generated by $Y \subseteq C$, then $B$ is generated by $f^\text{img}(X) \cup g^\text{pre}(Y)$.

---

##### _example:_ creating short exact sequences

1) If $x \in A$ is a non-zero-divisor of a commutative ring, then $A \to A$ by multiplication by $x$ is an injection. Thus, we can complete a short exact sequence with $A \to A / (x)$.
2) If $A \subseteq B$ is a submodule or $f : A \to B$ is an injection, then $A \to B \to B / A$ completes to a short exact sequence
3) If $g : M \to N$ is a surjection, then $\ker g \to M \to N$ completes to a short exact sequence
4) Any $h : A \to B$ gives $\ker h \to A \to \operatorname{img} h$ and $\operatorname{img} h \to B \to \operatorname{coker} h$ which both complete to short exact sequences.