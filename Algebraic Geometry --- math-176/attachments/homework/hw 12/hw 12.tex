...
\begin{solution}
	\begin{enumerate}
		\item Note that $\O$ is the set of primes $I \in \operatorname{Spec} \mathcal{O}$ that contain any maximal $M \in \operatorname{Spec} \mathcal{O}$ since, by definition, there are none. Thus, $\O$ is Zariski closed and its complement, $X$, is Zariski open.
			Similarly, note that $X = \operatorname{Spec} \mathcal{O} / \{ 0 \}$, and thus, $X$ is Zariski closed too. Thus, its complement, $\O$, is Zariski open.
		\item To show that the arbitrary union of open sets is open, we need to show that their complement is closed. By De Morgan's laws, an arbitrary union of open sets $U_{\alpha}$ has complement
			\[
				\left ( \bigcup_{\alpha \in \mathcal{I}} U_{\alpha} \right )^{c} = \bigcap_{\alpha \in \mathcal{I}} U_{\alpha}^{c}
			\]
			which is an arbitrary intersection of closed sets.
	\end{enumerate}
\end{solution}
