...
\begin{solution}
	\begin{enumerate}
		...
		\item Consider $A = \set{(x, y) \in \bb R^2 : y \ge 1/x, x > 0}$ and $B = \set{(x, y) \in \bb R^2 : y \le - 1/x, x > 0}$. Since $1/0$ is undefined, we do not consider any points for which $x = 0$ to be part of either set. \\\\
			Note that both sets are closed. For any $(x, y) \in \bb R^2 \setminus A$ there is a known technique to find the point on the curve $y = 1/x$ nearest to $(x, y)$. The neighbourhood $N_{(x, y)}(s)$ where $s$ is the distance of this nearest point is then contained in $\bb R^2 \setminus A$. Since neighbourhoods are open, this makes $\bb R^2 \setminus A$ open, and thus, $A$ closed. Similarly, we can show that $B$ is closed. Neither of them is compact since neither of them is bounded. \\\\
