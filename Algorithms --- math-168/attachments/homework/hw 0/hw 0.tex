...
\begin{solution}
	\begin{enumerate}
		...
		\item We prove for $p \in \mathbb{N}_{0}$ by induction. For $p = 0$ we have
			\begin{align*}
				\log_{b}(c^{0}) & = \log_{b} 1 \\
						& = 0 \\
						& = p \log_{b} c.
			...
			Now suppose we have $\log_{b} (c^{p}) = p \log_{b} c$ for some particular $p \in \mathbb{N}_{0}$. Then we can use the addition rule to write
			\begin{align*}
				\log_{b}(c^{p + 1}) & = \log_{b}(c^{p} c) \\
						    & = p \log_{b}(c) + \log_{b} c \\
						    & = (p + 1) \log_{b} c.
			\end{align*}

			Thus, by induction $\log_{b}(c^{p}) = p \log_{b} c$ for all $p \in \mathbb{N}_{0}$.
